...
```
  [Zorgverlener]  ──HTTPS──►  [OpenMRS Appointment Scheduling]
  [OpenMRS gebruiker]                      │
  [GitHub repository]  ──SSH/deploy──►  [Deployment servers]
```
...
| T2 | OpenMRS gebruiker → systeem | **Elevation of Privilege** | Elke geauthenticeerde gebruiker kan via de REST API afspraken van *alle* patiënten inzien — geen behandelaarcheck | R03 (IDOR) |
...
Op C2-niveau kijken we naar de grenzen en datastromen *tussen* de containers.

![Threat Model Niveau 2](../images/Threat%20Model%20Niveau%202.png)

```
  Browser ──► [OpenMRS WebApp (Tomcat)] ──► [Appointment API (Spring/Hibernate)]
                                                         │
                                              [MySQL Database]
                                                         │
  GitHub Actions ──► [GHCR / Docker image] ──► [Deployment server]
```
